...
import com.storecheckoutcounter.dataaccesslayer.repository.BillRepository;
...
public class BillService {
	...
	private Bill addProductToBill(Long billId, String barCodeId, int quantity) {
		...
		List<Item> currentLineItems = o1.getLineItems();
	...
	private void computeTotalValues(Bill bill) {
		...
		if (null != bill.getLineItems()) {
			List<Item> lineItems = bill.getLineItems();
	...
	private Item getLineItemWithBarCodeId(String barCodeId, List<Item> currentLineItems) {
		for (int i = 0; i < currentLineItems.size(); i++) {
			Item li = currentLineItems.get(i);
	...
	private Bill removeProductFromBill(Long billId, String barCodeId) {
		...
		if (currentLineItems != null && !currentLineItems.isEmpty()) {
			Item lineItem = getLineItemWithBarCodeId(barCodeId, currentLineItems);
